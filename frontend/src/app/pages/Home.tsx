...
function TiltCard({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className={`relative ${className}`} style={{ transformStyle: "preserve-3d" }}>
...
// ─── Floating Energy Orb ─────────────────────────────────────────────────────
function EnergyOrb({ size, color, delay, x, y }: { size: number; color: string; delay: number; x: string; y: string }) {
  return (
    <motion.div
...
export function Home() {
  ...
  const stats = [
    ...
    { value: "182.5", suffix: " kWh", label: "Lifetime Energy", icon: "🔋" },
    { value: "12,166", suffix: "", label: "Phone Charges", icon: "📱" },
  ];
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
              <motion.div variants={fadeUp}>
                ...
                >
                  <motion.span className="w-2 h-2 bg-aw-green rounded-full"
              ...
              <motion.div variants={fadeUp} className="flex flex-wrap gap-4">
                <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                  ...
                  >
                    <motion.span className="absolute inset-0 bg-aw-lime"
                      initial={{ x: "-100%" }}
                      whileHover={{ x: 0 }}
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {productSeries.map((series, index) => (
              <TiltCard key={index} className={`${series.color} text-white p-8 rounded-2xl cursor-pointer`}>
                <motion.div
