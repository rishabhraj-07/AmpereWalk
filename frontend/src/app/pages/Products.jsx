...
function TechRing({ label, value, color, angle }) {
  return <motion.div
...
function FlipCard({ product, index }) {
  ...
  >
    ...
    style={{
      ...
      transition: flipped ? "transform 0.7s cubic-bezier(0.22,1,0.36,1)" : void 0,
      transform: flipped ? "perspective(900px) rotateY(180deg)" : void 0
    }}
  ...
  >
          ...
          <ul className="space-y-2 flex-1">
            {product.features.map((f, i) => <li key={i} className="flex items-center gap-2 text-xs text-aw-dark-gray">
...
function BenefitCube({ benefit, index }) {
  ...
  >
    ...
    <motion.div className="absolute inset-0 bg-gradient-to-br from-aw-green/5 to-aw-teal/5 opacity-0 group-hover:opacity-100 transition-opacity duration-400" />
    <motion.p className="font-black text-aw-navy mb-1 relative z-10" style={{ transform: "translateZ(15px)" }}>
      {benefit.label}
    </motion.p>
    <p className="text-xs text-aw-green font-semibold relative z-10">{benefit.sub}</p>
  </motion.div>;
}
...
function MarketCard({ market, index }) {
  ...
  >
    <div className="tilt-shine absolute inset-0 rounded-2xl pointer-events-none opacity-0 z-10" />
    <motion.div className="absolute inset-0 bg-aw-navy opacity-0 group-hover:opacity-100 transition-opacity duration-350 rounded-2xl" />
...
function Products() {
  ...
  >
                {["Shoes That", "Work", "While You", "Walk"].map((word, i) => <motion.span
    key={i}
  ...
  >
                {["Generate Electricity", "Track Health", "Stay Safe"].map((tag, i) => <motion.span
    key={tag}
  ...
  >
              <div className="relative" style={{ perspective: 1e3 }}>
                {
    /* Rotating rings */
  }
                {[1.08, 1.16, 1.24].map((scale, i) => <motion.div
    key={i}
  ...
  >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {productSeries.map((product, index) => <FlipCard key={index} product={product} index={index} />)}
          </div>
        </div>
      </section>
  ...
  >
    ...
    /* 3D benefit cubes */
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {keyBenefits.map((benefit, index) => <BenefitCube key={benefit.label} benefit={benefit} index={index} />)}
          </div>
        </div>
      </section>
  ...
  ].map((step, index) => <TechStepCard key={step.step} step={step} index={index} />)}
          </div>
        </div>
      </section>
  ...
  >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-7">
            {targetMarkets.map((market, index) => <MarketCard key={market.title} market={market} index={index} />)}
          </div>
        </div>
      </section>
