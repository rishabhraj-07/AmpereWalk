    ...
    <div className="min-h-screen flex flex-col">
      ...
      <motion.header
        className={`sticky top-0 z-50 border-b transition-all duration-300 ${
          scrolled
            ? "bg-aw-navy/90 backdrop-blur-xl border-white/10 shadow-xl shadow-black/20"
            : "bg-aw-navy border-transparent"
        }`}
        initial={{ y: -80 }}
        animate={{ y: 0 }}
        ...
        <div className="mx-auto px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => (
                <NavLink key={link.path} to={link.path} label={link.label} isActive={isActive(link.path)} />
              ))}
            </nav>

            {/* CTA Buttons */}
            <div className="hidden md:flex items-center gap-3">
              <Link to="/login" className="px-4 py-2 text-white/70 hover:text-white text-sm font-medium transition-colors">
                Login
              </Link>
              <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                <Link to="/dashboard"
                  className="relative px-5 py-2 bg-aw-green text-white rounded-lg text-sm font-semibold overflow-hidden group"
                >
                  <motion.span
      ...
      {/* ── FOOTER ─────────────────────────────────────────────────────── */}
      <footer className="bg-aw-navy text-white relative overflow-hidden">
        {/* Subtle grid */}
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
            <div>
              ...
              <p className="text-sm text-white/50 mb-4">Power Every Step</p>
              {/* Animated energy bar */}
            ...
            <div>
              <h3 className="font-bold mb-5 text-sm uppercase tracking-widest text-white/60">Quick Links</h3>
              <ul className="space-y-3 text-sm">
                {[{ to: "/about", label: "About Us" }, { to: "/products", label: "Products" }, { to: "/careers", label: "Careers" }].map((l) => (
                  <li key={l.to}>
                    <Link to={l.to} className="text-white/50 hover:text-aw-green transition-colors flex items-center gap-2 group">
            ...
            <div>
              <h3 className="font-bold mb-5 text-sm uppercase tracking-widest text-white/60">Support</h3>
              <ul className="space-y-3 text-sm">
                {[{ to: "/contact", label: "Contact Us" }, { to: "/blog", label: "Blog" }].map((l) => (
                  <li key={l.to}>
                    <Link to={l.to} className="text-white/50 hover:text-aw-green transition-colors flex items-center gap-2 group">
                      <motion.span className="w-0 h-px bg-aw-green group-hover:w-3 transition-all duration-300" />
                      {l.label}
            ...
            <div>
              <h3 className="font-bold mb-5 text-sm uppercase tracking-widest text-white/60">Connect</h3>
              <p className="text-sm text-white/50 mb-4">Follow our journey</p>
              <motion.a
                href="https://linkedin.com/company/amperewalk-pvt-ltd/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-aw-green hover:text-aw-lime transition-colors text-sm font-semibold"
                whileHover={{ x: 4 }}
              >
                LinkedIn <span>→</span>
              </motion.a>
          ...
          <div className="border-t border-white/10 mt-12 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-white/30">
            <p>© 2026 AmpereWalk Pvt. Ltd. | NextGen Shoes | All rights reserved.</p>
            <motion.div
