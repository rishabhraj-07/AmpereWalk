...
function Layout() {
  ...
  const genericAuth = useAuth();
  const careerAuth = useCareerAuth();
  ...
  const isCareerRoute = location.pathname.startsWith("/careers");
  const dashboardLink = isCareerRoute ? "/careers/dashboard" : "/dashboard";
  const loginLink = isCareerRoute ? "/careers/login" : "/login";
  const currentAuth = isCareerRoute ? careerAuth : genericAuth;
  const isAuthed = currentAuth.isAuthenticated && !currentAuth.isInitializing;
  const handleLogout = () => {
    currentAuth.logout().finally(() => {
      navigate(isCareerRoute ? "/careers/login" : "/login", { replace: true });
    });
  };
  ...
  >
    ...
    /* CTA Buttons */
            ...
            <div className="hidden md:flex items-center gap-3">
              {!isAuthed && <Link to={loginLink} className="inline-flex min-w-20 items-center justify-center rounded-lg border border-white/15 px-4 py-2 text-sm font-semibold text-white/85 transition-colors hover:border-aw-green/60 hover:bg-white/10 hover:text-white">
                  Login
                </Link>}
  ...
  >
                ...
                {!isAuthed && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}>
                    <Link to={loginLink} className="block px-4 py-3 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
                      Login
                    </Link>
                  </motion.div>}
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
                  <Link to={dashboardLink} className="block px-4 py-3 bg-aw-green text-white rounded-lg hover:bg-aw-lime transition-colors text-center font-semibold">
                    Dashboard
                  </Link>
                </motion.div>
