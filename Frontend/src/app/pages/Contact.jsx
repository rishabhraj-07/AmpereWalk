...
function Contact() {
  ...
  >
                    ...
                    {reasons.map((reason, index) => <option key={index} value={reason}>
                        {reason}
                      </option>)}
                  </select>
                </div>
  ...
  />
    ...
    className="w-full px-6 py-4 bg-aw-green text-white rounded-lg hover:bg-aw-lime transition-colors flex items-center justify-center gap-2 group"
                ...
                {submitState === "sent" && <p className="text-sm text-aw-green font-semibold">
                  Thanks. Your message is ready for the AmpereWalk team.
                </p>}
              </form>
            </motion.div>
